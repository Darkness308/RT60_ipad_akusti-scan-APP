...
/// PDF report renderer that uses the central ReportModel
public final class PDFReportRenderer {
    ...
    private func drawContent(pageRect: CGRect, model: ReportModel) {
        ...
        for target in model.din_targets {
            let freq: String
            if let f = target["freq_hz"] {
                freq = String(Int(f.rounded()))
            } else {
                freq = "-"
            }
            
            let tsoll: String
            if let ts = target["t_soll"] {
                tsoll = String(format: "%.2f", ts)
            } else {
                tsoll = "-"
            }
            
            let tol: String
            if let tolerance = target["tol"] {
                tol = String(format: "%.2f", tolerance)
            } else {
                tol = "-"
            }
    ...
    public func render(_ model: ReportModel) -> Data {
        ...
        for target in model.din_targets {
            let f: String
            if let freq = target["freq_hz"] {
                f = String(Int(freq.rounded()))
            } else {
                f = "-"
            }
            
            let ts: String  
            if let tsoll = target["t_soll"] {
                ts = String(format: "%.2f", tsoll)
            } else {
                ts = "-"
            }
