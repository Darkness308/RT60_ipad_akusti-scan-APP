...
public enum EvaluationStatus: String, CaseIterable, Codable {
    ...
    /// RT60 values are mixed, but majority are acceptable (aggregate status)
    case partiallyCompliant = "partially_compliant"
    ...
    /// Indicator whether the status is compliant
    public var isCompliant: Bool {
        return self == .withinTolerance || self == .partiallyCompliant
    }
